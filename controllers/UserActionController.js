...
const dateAddMinutes = require('date-fns/add_minutes')
const dateAddMonths = require('date-fns/add_months')
const dateCompareAsc = require('date-fns/compare_asc')
...
class UserController {
  ...
  async authenticate(ctx) {
    ...
    try {
      let correct = await bcrypt.compare(request.password, userData.password)
      if (!correct) {
  ...
  async forgot(ctx) {
    ...
    if (request.type === 'web') {
      let email = await fse.readFile('../email/forgot.html', 'utf8')
      let resetUrlCustom =
        request.url +
        '?passwordResetToken=' +
